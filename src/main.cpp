...
#if defined(NDEBUG)
# error "Anubiscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb1536b0d5325a21196989576741bd64b0a59a96e64424f93586b61ff7ed73f01");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Anubiscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Anubiscoin Signed Message:\n";
...
{
    // Anubiscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Anubiscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Anubiscoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Anubiscoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Anubiscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Anubiscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb1536b0d5325a21196989576741bd64b0a59a96e64424f93586b61ff7ed73f01");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Guide of the obscure cryptcoins underworld! In 02/01/2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x4fc97a780f0c02f3828185f151b8bb4cb1e869a42295c5956c54c6aadf55cd67"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Anubiscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AnubiscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AnubiscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AnubiscoinMiner(CWallet *pwallet)
{
    printf("AnubiscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("anubiscoin-miner");
    ...
    try { loop {
        ...
        printf("Running AnubiscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AnubiscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AnubiscoinMiner, pwallet));
}
